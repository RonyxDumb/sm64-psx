...
SRC_DIRS += src/port src/port/gfx src/port/dma src/port/pc
...
#LEVEL_SCRIPT_O_FILES := $(LEVEL_SCRIPT_C_FILES:%.c=$(BUILD_DIR)/%.o) $(LEVEL_GEO_C_FILES:%.c=$(BUILD_DIR)/%.o)

GODDARD_O_FILES := $(GODDARD_C_FILES:%.c=$(BUILD_DIR)/%.o)

# Automatic dependency files
DEP_FILES := $(O_FILES:.o=.d) $(LEVEL_O_FILES:.o=.d) $(GODDARD_O_FILES:.o=.d)
...
TARGET_CFLAGS := -ggdb3 $(DEF_INC_CFLAGS) -fno-common -fsingle-precision-constant -D_LANGUAGE_C -UNDEBUG -fsanitize=undefined -fno-sanitize=shift -fsanitize=bounds-strict

TARGET_CFLAGS += -ffunction-sections -fdata-sections -Wl,--gc-sections

# C compiler options
TARGET_CFLAGS += -O0 -fno-ipa-icf -fno-omit-frame-pointer -fno-optimize-sibling-calls -fno-zero-initialized-in-bss
CFLAGS := -fno-lto
...
ALL_DIRS := $(BUILD_DIR) $(addprefix $(BUILD_DIR)/,$(SRC_DIRS) $(GODDARD_SRC_DIRS) $(BIN_DIRS) textures $(TEXT_DIRS) $(SOUND_SAMPLE_DIRS) $(addprefix levels/,$(LEVEL_DIRS)) rsp include)
...
$(OUTPUT): $(SEG_FILES_WITH_GEO) $(O_FILES) $(GODDARD_O_FILES) $(BUILD_DIR)/ext_files_defsym.txt
>	@$(PRINT) "$(GREEN)Linking: $(BLUE)$@$(NO_COL)\n"
>	$(V)$(LD) $(CFLAGS) -L $(BUILD_DIR) -no-pie -o $@ $(addprefix -Wl$(COMMA)--just-symbols=,$(SEG_FILES_WITH_GEO)) `cat $(BUILD_DIR)/ext_files_defsym.txt` $(O_FILES) $(GODDARD_O_FILES) $(LDFLAGS)
...
debug: $(OUTPUT)
>	@gdb -q -iex "set debuginfod enabled off" -ex run -- $<

.PHONY: all clean distclean default test debug

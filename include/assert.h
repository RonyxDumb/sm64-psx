...
#define assertm(expr, screen_msg) ((expr)? (void) 0: (printf("%s:%d: failed assert(%s)" screen_msg "\n", __FILE__, __LINE__, #expr), assert(false)))
#define assertmf(expr, screen_msg, ...) ((expr)? (void) 0: (printf("%s:%d: failed assert(%s)" screen_msg "\n", __FILE__, __LINE__, #expr, ##__VA_ARGS__), assert(false)))
#define abortf(msg, ...) assertmf(false, msg, ##__VA_ARGS__)

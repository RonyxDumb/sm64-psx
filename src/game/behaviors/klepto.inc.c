...
static void klepto_target_mario(void) {
    QSETFIELD(o, oKleptoDistanceToTarget, lateral_dist_between_objectsq(gMarioObject, o));
...
void bhv_klepto_init(void) {
    if (o->oBehParams2ndByte != 0) {
        ...
        QSETFIELD(o,  oKleptoStartPosX, QFIELD(o,  oPosX));
        QSETFIELD(o,  oKleptoStartPosY, QFIELD(o,  oPosY));
        QSETFIELD(o,  oKleptoStartPosZ, QFIELD(o,  oPosZ));
...
static void klepto_change_target(void) {
    ...
    if (QFIELD(o, oDistanceToMario) > q(2000.0)) {
        minTargetDist = 99999.0f;

        for (i = 0; i < 3; i++) {
            dx = FFIELD(gMarioObject, oPosX) - sKleptoTargetPositions[i][0];
            dz = FFIELD(gMarioObject, oPosZ) - sKleptoTargetPositions[i][2];
    ...
    FSETFIELD(o, oKleptoUnkF8, 400 * absi(newTarget - o->oKleptoTargetNumber));
    o->oKleptoTargetNumber = newTarget;

    FSETFIELD(o, oHomeX, sKleptoTargetPositions[o->oKleptoTargetNumber][0]);
    FSETFIELD(o, oHomeY, sKleptoTargetPositions[o->oKleptoTargetNumber][1] + FFIELD(o, oKleptoUnkF8));
    FSETFIELD(o, oHomeZ, sKleptoTargetPositions[o->oKleptoTargetNumber][2]);

    FSETFIELD(o, oKleptoUnkFC, cur_obj_lateral_dist_to_home() / 2);
}
...
static void klepto_circle_target(f32 radius, f32 targetSpeed) {
    ...
    if (o->oAnimState != KLEPTO_ANIM_STATE_HOLDING_NOTHING
        && ((o->oTimer > 60 && QFIELD(o, oDistanceToMario) > q(2000.0))
        ...
        turnAmount = 0x4000 - atan2s(radius, FFIELD(o, oKleptoDistanceToTarget) - radius);
        accel = 0.05f;
        ...
        turnAmount =
            (s16)(s32)(abs_angle_diff(o->oKleptoYawToTarget, o->oMoveAngleYaw) * (0.03f * FFIELD(o, oKleptoSpeed)));
        clamp_s16(&turnAmount, 400, 700);
        obj_rotate_yaw_and_bounce_off_walls(o->oKleptoYawToTarget, turnAmount);

        if (QFIELD(o, oKleptoSpeed) > q(50.0f)) {
            accel = 2.0f;
        }

        APPROACH_F32_FIELD(o, oKleptoSpeed, targetSpeed, accel);
    }
}

static void klepto_approach_targetq(q32 targetSpeedq) {
    if (QFIELD(o, oKleptoDistanceToTarget) < q(1800)) {
        o->oAction = KLEPTO_ACT_CIRCLE_TARGET_HOLDING;
    } else {
        if (QFIELD(o, oKleptoUnkFC) > 0) {
            if (QMODFIELD(o, oKleptoUnkFC, -= QFIELD(o, oForwardVel)) <= 0) {
                QMODFIELD(o, oHomeY, -= QFIELD(o, oKleptoUnkF8));
            }
        }

        klepto_anim_dive();
        obj_rotate_yaw_and_bounce_off_walls(o->oKleptoYawToTarget, 400);
        APPROACH_Q32_FIELD(o, oKleptoSpeed, targetSpeedq, q(0.05));
    }
}

static void klepto_act_wait_for_mario(void) {
    if (QFIELD(o, oKleptoDistanceToTarget) < q(1000.0f)) {
        klepto_target_mario();
        if (QFIELD(o, oKleptoDistanceToTarget) < q(1000.0f)) {
...
static void klepto_act_turn_toward_mario(void) {
    ...
    if (klepto_set_and_check_if_anim_at_end() && cur_obj_check_if_at_animation_end() && FFIELD(o, oKleptoDistanceToTarget) > 800.0f
...
static void klepto_act_dive_at_mario(void) {
    APPROACH_F32_FIELD(o, oKleptoSpeed, 60.0f, 10.0f);
    if (o->oSoundStateID == 2) {
        ...
        f32 dy = FFIELD(o, oPosY) - FFIELD(gMarioObject, oPosY);
        if (o->oSoundStateID == 3) {
            cur_obj_set_anim_if_at_end(4);
        } else if (FFIELD(o, oVelY) > 0.0f && dy > 200.0f) {
        ...
        if (o->oAnimState == KLEPTO_ANIM_STATE_HOLDING_NOTHING) {
            ...
            if (gMarioStates[0].action != ACT_SLEEPING
                ...
                && QFIELD(o, oDistanceToMario) < q(200) && dy > 50.0f && dy < 90.0f) {
...
static void klepto_act_struck_by_mario(void) {
    ...
    if (cur_obj_check_if_near_animation_end()) {
        ...
        FSETFIELD(o, oKleptoSpeed, sqrtf(FFIELD(o, oForwardVel) * FFIELD(o, oForwardVel) + FFIELD(o, oVelY) * FFIELD(o, oVelY)));

        QSETFIELD(o,  oHomeX, QFIELD(o,  oPosX));
        FSETFIELD(o, oHomeY, FFIELD(o, oPosY) + 500.0f);
...
static void klepto_act_retreat(void) {
    ...
    APPROACH_F32_FIELD(o, oKleptoSpeed, 40.0f, 10.0f);
...
static void klepto_act_reset_position(void) {
    if (o->oTimer < 300) {
        ...
        QSETFIELD(o,  oHomeX, q(-2000));
        QSETFIELD(o,  oHomeZ, q(-1000));
        FSETFIELD(o, oHomeY, QSETFIELD(o,  oKleptoDistanceToTarget, q(9999)));

        if (QFIELD(o, oPosY) > q(5000.0f)) {
            obj_mark_for_deletion(o);
        } else {
            klepto_approach_targetq(q(20.0f));
        }
    } else {
        o->oAction = KLEPTO_ACT_WAIT_FOR_MARIO;
        QSETFIELD(o,  oHomeX, QFIELD(o,  oKleptoStartPosX));
        QSETFIELD(o,  oHomeY, QFIELD(o,  oKleptoStartPosY));
        QSETFIELD(o,  oHomeZ, QFIELD(o,  oKleptoStartPosZ));
    }
}
...
void bhv_klepto_update(void) {
    ...
    FSETFIELD(o, oKleptoDistanceToTarget, cur_obj_lateral_dist_to_home());
    o->oKleptoUnk1B0 = obj_get_pitch_to_home(FFIELD(o, oKleptoDistanceToTarget));
    o->oKleptoYawToTarget = cur_obj_angle_to_home();

    if (o->oAction == KLEPTO_ACT_STRUCK_BY_MARIO) {
        klepto_act_struck_by_mario();
    } else {
        obj_compute_vel_from_move_pitchq(QFIELD(o, oKleptoSpeed));
        ...
        switch (o->oAction) {
            ...
            case KLEPTO_ACT_APPROACH_TARGET_HOLDING:
                klepto_approach_targetq(q(50.0f));
                break;
